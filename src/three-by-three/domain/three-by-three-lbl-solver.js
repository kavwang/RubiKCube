...
export function solveFastestPlan(scrambleState, engine) {
  ...
  // 1. Cross (Optimal)
  const crossStart = moves.length;
  const crossTargetIndices = [
    localIdx("U", 2, 1), localIdx("F", 0, 1),
    localIdx("U", 1, 2), localIdx("R", 0, 1),
    localIdx("U", 0, 1), localIdx("B", 0, 1),
    localIdx("U", 1, 0), localIdx("L", 0, 1)
  ];
  const crossResult = solveCrossOptimal(state, engine);
  if (!crossResult.ok) {
    return { ok: false, message: "最速解第一層十字求解失敗，請檢查顏色輸入。" };
  }
  state = engine.applyMoveTokens(state, crossResult.moves);
  moves.push(...crossResult.moves);
  for (const idx of crossTargetIndices) locked.add(idx);
  if (moves.length > crossStart) {
    phases.push({ name: "第一層：白色十字 (最速解)", startIdx: crossStart, endIdx: moves.length });
  }

  // 2. Corners (BFS per corner)
  const cornersStart = moves.length;
  for (let i = 0; i < LBL_TARGETS.firstLayerCorners.length; i += 1) {
    const target = LBL_TARGETS.firstLayerCorners[i];
    const targetColors = new Set(target.map((idx) => engine.SOLVED_FACELETS[idx]));
    ...
    const result = solveCornerBFS(state, target, [...locked], allowedFaces, engine);
    if (!result.ok) {
      return { ok: false, message: `最速解第一層角塊第 ${i + 1} 塊失敗。` };
    }
    state = engine.applyMoveTokens(state, result.moves);
    moves.push(...result.moves);
    for (const idx of target) locked.add(idx);
  ...
  if (moves.length > cornersStart) {
    phases.push({ name: "第一層：白色角塊 (最速解)", startIdx: cornersStart, endIdx: moves.length });
  }

  // 3. Second Layer Edges (Optimal BFS with fallback)
  const middleStart = moves.length;
  for (let i = 0; i < LBL_TARGETS.secondLayerEdges.length; i += 1) {
    const target = LBL_TARGETS.secondLayerEdges[i];
    const allowedFaces = [...CORNER_SLOT_SIDE_FACES[i], "D"];
    let result = solveSecondLayerEdgeOptimalBFS(state, target, [...locked], allowedFaces, engine);
    if (!result.ok) {
      result = solvePhase2BFS(state, target, [...locked], engine);
    }
    if (!result.ok) {
      return { ok: false, message: `最速解第二層稜塊第 ${i + 1} 塊失敗。` };
    }
    state = engine.applyMoveTokens(state, result.moves);
    moves.push(...result.moves);
    for (const idx of target) locked.add(idx);
  ...
  if (moves.length > middleStart) {
    phases.push({ name: "第二層：中層稜塊 (最速解)", startIdx: middleStart, endIdx: moves.length });
  }

  // 4. Last Layer (Two Stage)
  const llStart = moves.length;
  const llResult = solveLastLayerTwoStage(state, engine);
  if (!llResult.ok) {
    return { ok: false, message: "最速解第三層還原失敗，請檢查顏色輸入。" };
  }
  state = engine.applyMoveTokens(state, llResult.moves);
  moves.push(...llResult.moves);
  if (moves.length > llStart) {
    phases.push({ name: "第三層：頂層還原 (最速解)", startIdx: llStart, endIdx: moves.length });
  }

  if (!engine.isCubeSolved(state)) {
    return { ok: false, message: "最速解流程未收斂到復原狀態，請重新打亂再試。" };
  }
